...
NTSTATUS request_firmware(struct firmware** img, PCWSTR path) {
	*img = NULL;
	...
	struct firmware* fw = (struct firmware*)ExAllocatePoolZero(NonPagedPool, sizeof(struct firmware), CROSKBLIGHT_POOL_TAG);
	if (!fw) {
	...
	fw->data = (UINT8 *)ExAllocatePoolZero(NonPagedPool, fw->size, CROSKBLIGHT_POOL_TAG);
	if (!fw->data) {
...
void free_firmware(struct firmware* fw) {
	if (fw->data) {
		ExFreePoolWithTag(fw->data, CROSKBLIGHT_POOL_TAG);
	}
	ExFreePoolWithTag((PVOID)fw, CROSKBLIGHT_POOL_TAG);
}